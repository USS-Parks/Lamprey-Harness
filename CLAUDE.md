...
- `WebContentsView` (Electron 35) replaces deprecated `BrowserView` — uses DIP coordinates (no scaleFactor multiplication needed). Electron is pinned to ^35.7.5 because better-sqlite3 12.10 doesn't yet support V8 13 (Electron 36+). Bump when better-sqlite3 ships V8 13 compat.
  - **Tracked security debt:** this pin keeps `npm audit` showing one HIGH for `electron` (a batch of local-exploitation UAF/IPC advisories; the advisory range is `<=39.8.4`, so even the latest 35.x has no clean patch). Accepted as a desktop-app, bounded-surface risk for now. **Revisit trigger:** when better-sqlite3 publishes V8 13 / Electron 36+ support, bump Electron to the newest patched line and re-run `npm audit` — that single HIGH should clear. (The rest of the tree is clean as of the 2026-06-04 dependency-security pass: the `@xenova/transformers → onnx-proto → protobufjs` CRITICAL chain and the `electron-rebuild → tar` HIGH chain were both removed.)
