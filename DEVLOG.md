...
`https://open.bigmodel.cn/api/paas/v4/` (OpenAI-compatible). Two catalog entries: `glm-5.2`
(128K context) and `glm-5.2[1m]` (1M context), both with tool use + vision. The provider card
appears automatically in Settings → API Keys.

v0.15.2 shipped with `apiModelId: 'GLM-5-2'` which Zhipu rejected (400 "model does not exist").
The correct ID is `glm-5.2` (lowercase, dot-separated). v0.15.3 fixes the ID and adds the 1M
variant.
