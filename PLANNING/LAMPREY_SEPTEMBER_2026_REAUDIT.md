...
**SR-36A packaging correction:** The first 0.32.0 candidate (`dc67b05de22e79170d20baea0d0d2af5f12d965a`) passed its hosted gates but failed real packaged vector retrieval before tagging. SQLite received the logical `app.asar` DLL path and could not load the extension, producing zero vector hits while lexical results still appeared. `vec-loader.ts` now translates the archive segment to the physical `app.asar.unpacked` path. A fresh electron-builder package using the stock executable and unchanged ASAR passed isolated startup, real embedding, ingestion and vector retrieval. The dedicated native suite passed all 170 tests. [Working-tree package receipt](evidence/sr36a-unpacked.json) records the base SHA and ASAR hash; it is not a published-artifact receipt. Final tag artifact acceptance remains required in SR-37.
