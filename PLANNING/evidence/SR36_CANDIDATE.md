...
The exact candidate is the commit introducing this file, resolved by `git log -1 --format=%H -- PLANNING/evidence/SR36_CANDIDATE.md`. Its push to main triggers CI and all platform builds. SR-37 must verify those exact hosted results before Bucket creates the tag.
...
All source behavior was accepted at `ae0ab1467fd62fc4b3d219a99eb120a19bc963c9`; subsequent changes are release orchestration, documentation and version metadata. SR-35's seven release regressions and existing gate contracts pass. Candidate build/proof checks run again for the version change; source CI and final tag build remain required.
