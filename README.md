...
Grab the latest **Windows** build from the [Releases page](https://github.com/USS-Parks/lamprey/releases/latest): pick the `.exe` (standard NSIS installer) or the `.zip` (portable bundle — unzip and run `Lamprey.exe` directly, no install required).
...
1. **Download** the [latest installer](https://github.com/USS-Parks/lamprey/releases/latest) and run it.
