---
type: Architecture
title: Function Calling — Native and Fallback Pathways
description: Complete guide to tool dispatch, schema validation, provider normalization, and how to add tools or providers (v0.9.0+).
tags: [function-calling, tools, architecture, providers]
...
Every provider — 33 built-ins (DeepSeek, Google, DashScope, OpenRouter, Zhipu, OpenAI, Anthropic, xAI, Mistral, Moonshot, Groq, Together, Fireworks, Cerebras, Hugging Face router, Ollama, LM Studio, and others) plus any user-defined custom endpoint — is reached over the same OpenAI-compatible chat-completions wire with identical tool schemas. Per-provider constraints live in §16.
...
All providers use the same normalizer logic (identical accepted subsets per FC-0 audit).

**Invocation (WC-1, 2026-06-09):** The normalizer is reached through `toolRegistry.getNormalizedToolsForProvider(provider)` at `electron/services/tool-registry.ts:518`. The chat dispatch at `electron/ipc/chat.ts:467` calls it with the active provider so every outgoing tool list is normalized before reaching `chatStream` / the API. Tests in `electron/services/tool-registry.test.ts` assert the path is hot.
...
**Note (v0.30.0):** The Planner and Reviewer roles were deleted in the Unburdening Phase (v0.14.0); this section is historical for reference only.
...
- They are included in `getNormalizedToolsForProvider()` / `getModelToolSurface()` output
...
3. Add to appropriate `role-tool-access.ts` allowlist if restricted (note: Planner/Reviewer roles were deleted v0.14.0)
...
4. If the provider accepts a different schema subset, add per-provider logic to
   `schema-normalizer.ts` (none of the built-ins needs any)
...
| anthropic | none through compat | probe at first live key | Official compat layer at `api.anthropic.com/v1/` (trailing slash). Tools + streaming fully supported; `strict`, `response_format`, `reasoning_effort` silently ignored. Never set `reasoningCapOnToolUse` on anthropic models; `isReasoner` stays false. Anthropic commits to no breaking changes. |
| xai | — | yes | |
| mistral | — | yes | Catalog uses documented rolling `-latest` aliases |
| moonshot | `reasoning_content` (thinking models) | yes | API host stays `api.moonshot.ai` after the Kimi rebrand. `kimi-k2-thinking` carries the full reasoning guard. |
| groq | — | yes | Ids may be vendor-prefixed (`openai/gpt-oss-120b`) |
| together | — | yes | Hub-style ids (`meta-llama/...`) |
| fireworks | — | yes | `accounts/fireworks/models/...` ids; shipped entry is unverified until a key exists |
| cerebras | — | yes | 403 (not 401) on unauthenticated probes |
| huggingface | — | yes (public, keyless) | Hub ids with optional `:fastest`/`:cheapest`/`:provider` routing suffix |
| ollama / lmstudio | model-dependent | yes when running | Keyless (`keyOptional`); placeholder key `'local'` sent. Empty built-in catalogs — import live ids. `providerBaseUrlOverrides` covers LAN hosts/ports. |
| custom endpoints | unknown | endpoint-dependent | settings.json `customProviders`; validated at the registry; `requiresKey` default false |

---

## 17. Function Calling Dispatch Diagram
...
Further reading: [domains/tools/catalog.md](../domains/tools/catalog.md), [domains/tools/mcp.md](../domains/tools/mcp.md), [architecture/overview.md](overview.md)
