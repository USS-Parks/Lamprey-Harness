...
async function main() {
  ...
  try {
    ...
    if (process.argv.includes('--keyboard-only')) {
      const result = await require('./ux-keyboard.cjs')({ page, app, ids, trackPid: pid => { if (pid) fixturePids.push(pid) } })
      fs.writeFileSync(path.join(output, 'KEYBOARD.json'), JSON.stringify(result, null, 2) + '\n')
      fs.writeFileSync(path.join(output, 'RUNTIME.json'), JSON.stringify({ scope: 'keyboard-only', runtime: await app.evaluate(() => process.versions), seeded, completed: ['keyboard-navigation'] }, null, 2) + '\n')
    ...
    if (process.argv.includes('--navigation-only')) {
      ...
      fs.writeFileSync(path.join(output, 'RUNTIME.json'), JSON.stringify({ scope: 'navigation-only', runtime: await app.evaluate(() => process.versions), seeded, completed: ['task-navigation'] }, null, 2) + '\n')
      lifecycle.passed = true
      return
    }
    if (process.argv.includes('--task-data-only')) {
      const result = await require('./ux-task-data.cjs')({ page, ids, repo })
      fs.writeFileSync(path.join(output, 'TASK_DATA.json'), JSON.stringify(result, null, 2) + '\n')
      fs.writeFileSync(path.join(output, 'RUNTIME.json'), JSON.stringify({ scope: 'task-data-only', runtime: await app.evaluate(() => process.versions), seeded, completed: ['task-data'] }, null, 2) + '\n')
      lifecycle.passed = true
      return
    }
    ...
    if (process.argv.includes('--attention-only')) {
      ...
      fs.writeFileSync(path.join(output, 'RUNTIME.json'), JSON.stringify({ scope: 'task-attention-only', runtime: await app.evaluate(() => process.versions), seeded, completed: ['task-attention'] }, null, 2) + '\n')
      lifecycle.passed = true
      return
    }
    if (process.argv.includes('--history-only')) {
      const result = await require('./ux-history.cjs')({ page, app, ids, providerBodies, finishStreams: () => [...streamFinishers].forEach(finish => finish()) })
      fs.writeFileSync(path.join(output, 'HISTORY.json'), JSON.stringify(result, null, 2) + '\n')
      fs.writeFileSync(path.join(output, 'RUNTIME.json'), JSON.stringify({ scope: 'tool-history-only', runtime: await app.evaluate(() => process.versions), seeded, completed: ['tool-history'] }, null, 2) + '\n')
      lifecycle.passed = true
      return
    }
    ...
    const history = await require('./ux-history.cjs')({ page, app, ids, providerBodies, finishStreams: () => [...streamFinishers].forEach(finish => finish()) })
    fs.writeFileSync(path.join(output, 'HISTORY.json'), JSON.stringify(history, null, 2) + '\n')
    record('tool-history')
    ...
    const taskData = await require('./ux-task-data.cjs')({ page, ids, repo })
    fs.writeFileSync(path.join(output, 'TASK_DATA.json'), JSON.stringify(taskData, null, 2) + '\n')
    record('task-data')
