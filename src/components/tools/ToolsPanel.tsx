...
export function ToolsPanel({ onCollapse }: ToolsPanelProps) {
  ...
  return (
    <>
      ...
      {adding && <div role="group" aria-label="Add workspace resource" onKeyDown={event => { if (event.key === 'Escape' && !event.nativeEvent.isComposing) { event.preventDefault(); event.stopPropagation(); setAdding(false); add.current?.focus() } }} className="flex shrink-0 flex-wrap gap-1 border-b border-[var(--panel-border)] p-2">
